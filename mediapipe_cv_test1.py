...
def detect_emotion(face_landmarks, img_shape):
    ...
    # Surprise: Mouth openness and eyebrow height
    ...
    mouth_openness = np.linalg.norm(np.array(upper_lip) - np.array(lower_lip)) / mouth_width if mouth_width > 0 else 0
    ...
    eyebrow_raise = (left_eye_top[1] - left_eyebrow_top[1]) / mouth_width if mouth_width > 0 else 0

    if mouth_openness > 0.4 and eyebrow_raise > 0.1:
        return "Surprise"

    # Anger: Eyebrows drawn together
    ...
    eyebrow_furrow = np.linalg.norm(np.array(left_inner_eyebrow) - np.array(right_inner_eyebrow)) / mouth_width if mouth_width > 0 else 0

    if eyebrow_furrow < 0.25:
        return "Anger"
        
    # Happiness / Sadness: Mouth corner height relative to mouth center
    mouth_center_y = (upper_lip[1] + lower_lip[1]) / 2
    mouth_corners_y = (left_mouth_corner[1] + right_mouth_corner[1]) / 2
    smile_score = (mouth_center_y - mouth_corners_y) / mouth_width if mouth_width > 0 else 0
